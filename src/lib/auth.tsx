...
// Username-only auth: map username -> synthetic email (strict ascii).
// Premium/decorated/Arabic usernames are NOT allowed at signup.
// They are only granted via admin panel or via the paid "premium" upgrade.
...
export async function signUpWithUsername(username: string, password: string) {
  ...
  if (!/^[a-zA-Z0-9_]{3,20}$/.test(clean)) {
    return { error: "اسم المستخدم: 3-20 حرفًا، إنجليزي وأرقام و _ فقط (الأسماء المميزة تتطلب ترقية)" };
  }
