  ...
  return (
    <main className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 border-b border-border bg-background/90 px-5 py-4 backdrop-blur">
        <button onClick={() => navigate({ to: "/app" })} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" value={roomName} onChange={(e) => setRoomName(e.target.value)}
              placeholder="مثال: غرفة الأصدقاء" maxLength={50} required
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="وصف الغرفة..." rows={3} maxLength={200}
          ...
          <div>
            ...
            <input type="range" min={5} max={200} value={maxMembers}
              onChange={(e) => setMaxMembers(parseInt(e.target.value))} className="w-full" />
          </div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : (
              <span className="flex items-center justify-center gap-2"><CheckCircle className="h-4 w-4" /> إنشاء الغرفة</span>
            )}
          </button>
        </form>
