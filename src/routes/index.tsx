...
function RoomsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (roomsError) {
        console.error("خطأ في جلب الغرف:", roomsError);
        setError(roomsError.message);
        setLoading(false);
        return;
      }
      
      console.log("الغرف التي تم جلبها:", roomsData);
      ...
      if (countsError) {
        console.error("خطأ في جلب عدد الأعضاء:", countsError);
      }
      ...
      console.log("الغرف بعد إضافة عدد الأعضاء:", roomsWithCount);
